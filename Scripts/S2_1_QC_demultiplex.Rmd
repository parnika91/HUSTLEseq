...
# for Naive, to include more cells from raw eatures 
...
data <- Read10X_h5(filename =  paste0("../Data/",lib_name,"/h5/cellbender_seurat.h5"))
...
<!-- ```{r} -->
<!-- # for manually filtered naive library: -->
<!-- hustle <- readRDS("robjects/D13_D33_D45_D47_CD4_Naive_filtered_17k.rds") -->
...
hustle.hto <- as.matrix(hto[,joint.bcs]) #hustle.hto <- as.matrix(hustle[["HTO"]]@counts[,joint.bcs])
...
#hustle.singlet.donor <- MULTIseqDemux(hustle.singlet.donor, assay = "HTO") # for naive
RidgePlot(hustle.singlet.donor, assay = "HTO", features = rownames(hustle.singlet.donor[["HTO"]]), ncol = 4)

hustle.singlet.donor@meta.data <- hustle.singlet.donor@meta.data %>%
  dplyr::rename(HTO_classification.global = MULTI_classification)
```
...
deadcell_nF_uppercutoff <- 4000
deadcell_nC_lowercutoff <- 150
...
hustle@meta.data <- hustle@meta.data %>%
  dplyr::rename(HTO_classification = HTO_classification.global)
...
#   #group_by(donor_id) %>% 
#   count(Donor_expt, donor_id) #Donor_expt, donor_id
...
```{r}
#saveRDS(hustle, file = paste0("robjects/", lib_name, "_demultiplexed.rds"))
```
